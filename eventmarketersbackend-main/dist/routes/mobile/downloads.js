 ...
 */
router.post('/track', [
    (0, express_validator_1.body)('mobileUserId').notEmpty().withMessage('Mobile User ID is required'),
    (0, express_validator_1.body)('resourceId').notEmpty().withMessage('Resource ID is required'),
    (0, express_validator_1.body)('resourceType').isIn(['TEMPLATE', 'VIDEO', 'GREETING', 'CONTENT']).withMessage('Resource type must be TEMPLATE, VIDEO, GREETING, or CONTENT'),
    (0, express_validator_1.body)('fileUrl').optional().isString().withMessage('File URL must be a string'),
], async (req, res) => {
    try {
 ...
 */
router.get('/user/:mobileUserId', async (req, res) => {
    try {
        ...
        // Build where clause
        let whereClause = { mobileUserId: mobileUserId };
        if (type) {
