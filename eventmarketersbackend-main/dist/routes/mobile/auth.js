...
router.post('/register', async (req, res) => {
    try {
        const { deviceId, name, email, password, phone, appVersion, platform, fcmToken, companyName, alternatePhone, displayName, category, address, companyLogo, description, website } = req.body;
        if (!deviceId) {
...
router.post('/login', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({
            ...
            deviceId: mobileUser.deviceId,
            userType: 'MOBILE_USER'
        }, process.env.JWT_SECRET, { expiresIn: rememberMe ? '30d' : '7d' });
        res.json({
            success: true,
 ...
 */
router.post('/logout', extractUserId, async (req, res) => {
    try {
