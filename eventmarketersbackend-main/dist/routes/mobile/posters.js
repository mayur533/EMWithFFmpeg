...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        if (userProfiles.length > 0) {
            category = userProfiles[0].category;
 ...
 */
router.post('/like', [
    (0, express_validator_1.body)('posterId').notEmpty().withMessage('Poster ID is required'),
 ...
 */
function getMockPostersByCategory(category) {
    const basePosters = [
        {
            id: '1',
...
router.get('/', async (req, res) => {
    try {
        ...
        let posters = [];
        if (category) {
            posters = getMockPostersByCategory(category);
        }
        else {
