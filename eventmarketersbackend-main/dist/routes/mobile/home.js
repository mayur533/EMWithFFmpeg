 ...
 */
router.get('/video-content', async (req, res) => {
    try {
        ...
        if (duration === 'short')
            ...
            where.duration = { gte: 61, lte: 300 };
        else if (duration === 'long')
            where.duration = { gte: 301 };
...
router.get('/stats', async (req, res) => {
    try {
        const [totalTemplates, totalVideos, totalGreetings, templateDownloads, videoDownloads, greetingDownloads, templateLikes, videoLikes, greetingLikes] = await Promise.all([
