 ...
 */
router.get('/status', extractUserId, async (req, res) => {
    try {
 ...
 */
router.post('/subscribe', extractUserId, async (req, res) => {
    try {
        const userId = req.userId;
 ...
 */
router.post('/renew', extractUserId, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
 ...
 */
router.post('/cancel', extractUserId, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
...
router.post('/', async (req, res) => {
    try {
        const { mobileUserId, planId, amount, paymentId, paymentMethod = 'razorpay' } = req.body;
