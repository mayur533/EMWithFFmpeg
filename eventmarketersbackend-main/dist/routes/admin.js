...
// ============================================
// Get all subadmins
router.get('/subadmins', auth_1.requireAdmin, async (req, res) => {
    try {
...
// Create new subadmin
router.post('/subadmins', auth_1.requireAdmin, [
...
// Update subadmin
router.put('/subadmins/:id', auth_1.requireAdmin, [
    ...
    try {
        ...
        // mobileNumber field not available in schema
        if (role)
            updateData.role = role;
        if (permissions)
            updateData.permissions = JSON.stringify(permissions);
        if (assignedBusinessCategories)
            updateData.assignedBusinessCategories = JSON.stringify(assignedBusinessCategories);
        if (status)
...
// Delete subadmin
router.delete('/subadmins/:id', auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        // Check if subadmin exists
        const existingSubadmin = await prisma.subadmin.findUnique({
...
// ============================================
// Get all business categories (accessible by both admin and subadmin)
...
router.post('/business-categories', auth_1.requireStaff, [
...
router.put('/business-categories/:id', auth_1.requireStaff, [
...
router.delete('/business-categories/:id', auth_1.requireStaff, async (req, res) => {
    try {
        const { id } = req.params;
        // Check if category exists
...
router.post('/customers/:customerId/activate-subscription', auth_1.requireAdmin, [
    ...
    try {
        ...
        const subscription = await prisma.subscription.create({
            data: {
                customerId,
                plan,
                planId: plan === 'YEARLY' ? 'yearly-plan' : 'monthly-plan',
                status: 'ACTIVE',
                startDate,
                endDate
            }
...
router.post('/customers/:customerId/deactivate-subscription', auth_1.requireAdmin, async (req, res) => {
    try {
        const { customerId } = req.params;
        // Check if customer exists
        const customer = await prisma.customer.findUnique({
...
router.get('/customers/:customerId/subscription', auth_1.requireAdmin, async (req, res) => {
    try {
        const { customerId } = req.params;
        const customer = await prisma.customer.findUnique({
            where: { id: customerId },
