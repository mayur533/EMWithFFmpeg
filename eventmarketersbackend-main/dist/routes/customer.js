    ...
    try {
        ...
        const customer = await prisma.customer.create({
            data: {
                ...
                selectedBusinessCategory: businessCategory || 'General',
        ...
        if (autoActivateSubscription && subscriptionPlan && subscriptionAmount) {
            subscription = await prisma.subscription.create({
                data: {
                    ...
                    planId: subscriptionPlan === 'YEARLY' ? 'yearly-plan' : 'monthly-plan',
                    status: 'ACTIVE',
                    startDate: new Date(),
    ...
    try {
        ...
        if (search) {
            where.OR = [
                ...
                { selectedBusinessCategory: { contains: search, mode: 'insensitive' } }
            ];
        }
        if (status) {
        ...
        if (category) {
            where.selectedBusinessCategory = { contains: category, mode: 'insensitive' };
        }
...
router.put('/:id', auth_1.authenticateToken, auth_1.requireAdmin, [
    ...
    (0, express_validator_1.body)('selectedBusinessCategory').optional().isLength({ min: 2 }).withMessage('Business category must be at least 2 characters')
], async (req, res) => {
    try {
