  ...
  try {
    ...
    const customer = await prisma.customer.create({
      data: {
        ...
        appVersion: '1.0.0', // Default version for web-created customers
...
router.put('/:id', authenticateToken, requireAdmin, [
  ...
  body('businessPhone').optional().isLength({ min: 10 }).withMessage('Business phone must be at least 10 characters'),
  body('businessWebsite').optional().isURL().withMessage('Valid website URL required'),
  body('businessCategory').optional().isLength({ min: 2 }).withMessage('Business category must be at least 2 characters')
