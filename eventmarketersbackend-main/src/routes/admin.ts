...
router.post('/customers/:customerId/deactivate-subscription', async (req: Request, res: Response) => {
  try {
    const { customerId } = req.params;
...
router.get('/customers/:customerId/subscription', async (req: Request, res: Response) => {
  try {
    const { customerId } = req.params;
